...
export default function NewAppointmentForm({
  ...
  const [hasShownWeeklyWarning, setHasShownWeeklyWarning] = useState(false);
  ...
  const checkWeeklyAppointments = (scheduledAt: Date) => {
    const weekStart = startOfWeek(scheduledAt, { weekStartsOn: 1, locale: ptBR });
    const weekEnd = endOfWeek(scheduledAt, { weekStartsOn: 1, locale: ptBR });

    console.log("Verificando semana de:", scheduledAt.toISOString());
    console.log("Início da semana:", weekStart.toISOString());
    console.log("Fim da semana:", weekEnd.toISOString());
    ...
    const existingAppointment = existingAppointments.find(appointment => {
      ...
      const isSameWeek = appointmentDate >= weekStart && appointmentDate <= weekEnd;
      console.log(
        `Comparando ${appointmentDate.toISOString()} - Está na mesma semana? ${isSameWeek}`
      );
      return isSameWeek && appointment.clientId === userId;
    });
  ...
  const handleDateSelect = (date: Date | undefined) => {
    if (date) {
      ...
      if (selectedHour) {
        ...
        if (!hasShownWeeklyWarning) {
          const existingAppointment = checkWeeklyAppointments(updatedDate);
          if (existingAppointment) {
            const existingDate = new Date(existingAppointment.scheduledAt);
            const formattedDate = format(existingDate, "dd/MM/yyyy 'às' HH:mm", { locale: ptBR });
            toast.warning(
              `Você já tem um agendamento nesta semana no dia ${formattedDate}. Que tal marcar no mesmo dia em outro horário?`,
              { position: "top-center", duration: 10000 }
            );
            setHasShownWeeklyWarning(true);
          }
        }
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setHasShownWeeklyWarning(false);
      onClose();
    } catch (error) {
      setError(
