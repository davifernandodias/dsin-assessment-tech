...
describe("User tests", () => {
  it("should create a user or see if a user with role 'Admin' already exists with the mock data", async () => {
    try {
      ...
      console.log(
        "error no teste de criar usuário com dados mock ou verificar se já existe usúario com dados do mock: ",
        error
      );
    }
  });
  ...
  it("should return all users 'role: Admin' ", async () => {
    try {
      const response = await request(app).get(
        `/api/users?initial=0&limit=100&userId=${UserMockAdmin.id}`
      );
      expect(Array.isArray(response.body)).toBe(true);
      expect(response.body.length).toBeGreaterThan(0);
    } catch (error) {
      console.log(
        "error no teste de buscar todos os usuários por id com role 'Admin' : ",
        error
      );
    }
  });
  it("should return myself user information, only based on the id passed in the req params 'role: Client' ", async () => {
    try {
      const response = await request(app).get(
        `/api/users?initial=0&limit=100&userId=${UserMockClient.id}`
      );
      expect(Array.isArray(response.body)).toBe(true);
      expect(response.body.length).toBeGreaterThan(0);
      ...
      console.log(
        "error no teste de buscar todos os usuários por id com role 'Client' : ",
        error
      );
    }
  });
